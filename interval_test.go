...
func generateMinuteTests() (cases []TestCase) {
	caseBase := TestCase{
...
func TestCalculateNextExecutionTime(t *testing.T) {
	tests := []TestCase{
		...
		},
		{
			...
			expectedNextExecution: time.Date(2023, 1, 1, 10, 0, 0, 0, time.UTC),
			expectedWaitDuration:  time.Hour + 30*time.Minute,
		},
		{
			...
			expectedNextExecution: time.Date(2023, 1, 2, 10, 0, 0, 0, time.UTC),
			expectedWaitDuration:  17*time.Hour + 30*time.Minute,
		},
		...
		},
		{
			...
			now:                   time.Date(2023, 1, 1, 8, 30, 0, 0, time.UTC),
			expectedNextExecution: time.Date(2023, 1, 1, 10, 00, 0, 0, time.UTC),
			expectedWaitDuration:  time.Hour + 30*time.Minute,
		},
		{
		...
		{
			name: "First execution with both hour and minute intervals (current time within hour interval and within minute interval)",
			interval: Interval{
				hourSet:                true,
				startHour:              10,
				endHour:                14,
			...
			expectedNextExecution: time.Date(2023, 1, 1, 12, 20, 0, 0, time.UTC),
			expectedWaitDuration:  0,
		},
		{
			name: "First execution with both hour and minute intervals (current time within hour interval and before minute interval)",
			interval: Interval{
				hourSet:                true,
				startHour:              10,
				endHour:                14,
		...
		{
			name: "First execution with both hour and minute intervals (current time within hour interval and after minute interval)",
			interval: Interval{
				hourSet:                true,
				startHour:              10,
				endHour:                14,
		...
		},
		{
			...
			now:                   time.Date(2023, 1, 1, 12, 10, 0, 0, time.UTC),
			expectedNextExecution: time.Date(2023, 1, 1, 12, 10, 0, 0, time.UTC),
			expectedWaitDuration:  0,
		},
		{
		...
		// Minute + Every combinations
		{
			...
			now:                   time.Date(2023, 1, 1, 12, 20, 0, 0, time.UTC),
			expectedNextExecution: time.Date(2023, 1, 1, 13, 15, 0, 0, time.UTC),
			expectedWaitDuration:  55 * time.Minute,
		},
		{
			...
			now:                   time.Date(2023, 1, 1, 12, 10, 0, 0, time.UTC),
			expectedNextExecution: time.Date(2023, 1, 1, 12, 45, 0, 0, time.UTC),
			expectedWaitDuration:  5 * time.Minute,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			nextExec, waitDuration, err := tt.interval.calculateNextExecutionTime(tt.now)
