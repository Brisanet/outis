...
func WithHours(start, end uint) Option {
	return func(ctx *ContextImpl) { ctx.period.startHour, ctx.period.endHour = start, end }
}

// WithMinutes sets the start and end minutes of script execution
func WithMinutes(start, end uint) Option {
	return func(ctx *ContextImpl) { ctx.period.startMinute, ctx.period.endMinute = start, end }
}
...
func WithExecuteFirstTimeNow() Option {
	return func(ctx *ContextImpl) { ctx.executeFirstTimeNow = true }
}
